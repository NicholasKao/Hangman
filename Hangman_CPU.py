def hangman():
    ...
    def shorts(filename = 'dictionary.csv'): #58110 entries in the dictionary used
        infile = open(filename,'r')
        W = infile.read().split()
        infile.close()
        return(W)

    total = shorts(filename = 'dictionary.csv')

    
    import random
    word = total[random.randint(0,len(total))].lower() #word to play with in lower case
    level = input('Please select a level; easy, medium, or hard? ').lower() # selecting a level, which corresponds to allowed strikes
    ...
    #imaplementation of above function
    selectWord(word)

    # formatting so player2 cannot see player 1's word and showing strikes left:
    ...
    def showSize(letters):
        ...
        for x in letters: # iterates through the characters in the word
            if x.isalpha() == True: #if the character is a letter
                size.append('-') # then add an underscore to the 'board'
            else: #otherwise
    ...
    while strikes > 0: #while player 2 has strikes left
        if '-' not in board:
            ...
            if response == 'y':
                ...
                break
        letter = input('Please guess again: ') #prompt the user to guess
        ...
        if letter in possibles: #if the guess is in the letters left
            ...
            if letter in letters: #if the guess is in the word
                ...
                if strikes == 0: # if player 2 has no strikes left
                    ...
                    print('Computer wins! The word is ' + word)
                    break #end the game
        ...
        new_board = ' '.join(newBoard(letters, positions, letter))
        print('\n')
        print(new_board)
        print('\n')
